...
public class StringSList {
    ...
    // VERIFICA SE LA STRINGA INSERITA APPARTIENE ALLA LISTA
    public boolean belong(String s){
        if (isNull()) return false;
        if (car() == s) return true;
        return cdr().belong(s);
    }

    // VERIFICA SE LA STRINGA è PRESENTE come SOTTOSTRINGA DI QUALCHE VALORE
    public boolean contiene(String p){
        if (isNull()) return false;
        if (car().contains(p)) return true;
        return cdr().contiene(p);
    }
