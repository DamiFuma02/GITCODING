...
(define sequences ; val: lista di stringhe
     (lambda (n j) ; n, j: interi non negativi
         (if (> n 0)
             (let ( (u (sequences (- n 1) j)) ;lista di tanti elementi con lunghezza 2 
                    (v (if (> j 0)
                          (sequences (- n 1) (- j 1))   ;deve inserire un "-"
                          null  ;NON PUOI AGGIUNGERE "-"
                      )
                    )
                   )
             (append 
                   (map (lambda (s) (string-append "+" s) ) u) 
                   (map (lambda (s) (string-append "-" s) ) v)
                 )
             )
             (list "");SONO FINITI I CARATTERI DA AGGIUNGERE CASO DI USCITA
         )
   )
  )
