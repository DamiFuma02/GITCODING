...
;; Slightly modified by Claudio Mirolo (10th November 2002)
;; in order to provide tools for drawing towers of Hanoi.


(module hanoi mzscheme
 (provide get-default-image-size set-default-image-size!
   white-box image-width image-height
   line quarter-turn-right mirror-image invert
   overlay resize-image stack filled-triangle
   filled-ellipse filled-rectangle write-image-as-epsf)
 ...
 (define white-box
   (lambda  wh
     (let ((width default-image-size)
       ...
       (make-object fungraph-snip% width height))))
...
(define qtr-fungraph-snip%
 ...
 (define fe-fungraph-snip%
   (class100 fungraph-snip% (ishadow ix0 iy0 ix1 iy1 w h)
          (private-field
           (shadow ishadow)
           (x0 ix0)
           (y0 iy0)
          ...
          (override
           ...
           (fungraph-draw
            (lambda (dc xt yt body-pen body-brush other-pen other-brush)
              (send dc set-pen body-pen)
              (if shadow
                  (send dc set-brush other-brush)
                  (send dc set-brush body-brush))
              (let ((x0dc (xt x0 y0))
                    (y0dc (yt x0 y0))
                    (x1dc (xt x1 y1))
                    (y1dc (yt x1 y1)))
                (send dc draw-ellipse x0dc y0dc
                                      (abs (- x1dc x0dc)) (abs (- y1dc y0dc))))))
           (copy
            (lambda ()
              (let ((new (make-object fe-fungraph-snip% shadow x0 y0 x1 y1
                                      (get-width) (get-height))))
                (send new set-style (get-style))
                new))))
          (sequence
 ...
 (define filled-ellipse
   (lambda (shadow x0 y0 x1 y1 . wh)
     (if (not (real? x0))
         (error "x0 argument to filled-ellipse not a real" x0))
     (if (not (<= -1 x0 1))
         (error "x0 argument to filled-ellipse not in -1 to 1 range" x0))
     (if (not (real? x1))
         (error "x1 argument to filled-ellipse not a real" x1))
     (if (not (<= -1 x1 1))
         (error "x1 argument to filled-ellipse not in -1 to 1 range" x1))
     (if (not (real? y0))
         (error "y0 argument to filled-ellipse not a real" y0))
     (if (not (<= -1 y0 1))
         (error "y0 argument to filled-ellipse not in -1 to 1 range" y0))
     (if (not (real? y1))
         (error "y1 argument to filled-ellipse not a real" y1))
     (if (not (<= -1 y1 1))
         (error "y1 argument to filled-ellipse not in -1 to 1 range" y1))
     (let ((width default-image-size)
           (height default-image-size))
       (if (not (null? wh))
           (begin (set! width (car wh))
                  (if (not (null? (cdr wh)))
                      (begin (set! height (cadr wh))
                             (if (not (null? (cddr wh)))
                                 (error "too many argument to filled-ellipse")))
                      (set! height width))))
       (if (not (and (integer? height)
                     (integer? width)
                     (exact? height)
                     ...
                     (> height 0)
                     (> width 0)))
           (error "illegal size specification in filled-ellipse" wh))
       (make-object fe-fungraph-snip% shadow x0 y0 x1 y1 width height))))

  (define fr-fungraph-snip%
   (class100 fungraph-snip% (ishadow ix0 iy0 ix1 iy1 w h)
          (private-field
           (shadow ishadow)
           (x0 ix0)
           ...
           (fungraph-draw
            (lambda (dc xt yt body-pen body-brush other-pen other-brush)
              ...
              (let ((x0dc (xt x0 y0))
                    ...
                    (x1dc (xt x1 y1))
                    (y1dc (yt x1 y1)))
                (send dc draw-rectangle x0dc y0dc
                                        (abs (- x1dc x0dc)) (abs (- y1dc y0dc))))))
           (copy
            (lambda ()
              (let ((new (make-object fr-fungraph-snip% shadow x0 y0 x1 y1
                                      (get-width) (get-height))))
                (send new set-style (get-style))
                new))))
          (sequence
 ...
 (define filled-rectangle
   (lambda (shadow x0 y0 x1 y1 . wh)
     (if (not (real? x0))
         (error "x0 argument to filled-rectangle not a real" x0))
     (if (not (<= -1 x0 1))
         (error "x0 argument to filled-rectangle not in -1 to 1 range" x0))
     (if (not (real? x1))
         (error "x1 argument to filled-rectangle not a real" x1))
     (if (not (<= -1 x1 1))
         (error "x1 argument to filled-rectangle not in -1 to 1 range" x1))
     (if (not (real? y0))
         (error "y0 argument to filled-rectangle not a real" y0))
     (if (not (<= -1 y0 1))
         (error "y0 argument to filled-rectangle not in -1 to 1 range" y0))
     (if (not (real? y1))
         (error "y1 argument to filled-rectangle not a real" y1))
     (if (not (<= -1 y1 1))
         (error "y1 argument to filled-rectangle not in -1 to 1 range" y1))
     (let ((width default-image-size)
           (height default-image-size))
       (if (not (null? wh))
           (begin (set! width (car wh))
                  (if (not (null? (cdr wh)))
                      (begin (set! height (cadr wh))
                             (if (not (null? (cddr wh)))
                                 (error "too many argument to filled-rectangle")))
                      (set! height width))))
       (if (not (and (integer? height)
                     (integer? width)
                     (exact? height)
                     ...
                     (> height 0)
                     (> width 0)))
           (error "illegal size specification in filled-rectangle" wh))
       (make-object fr-fungraph-snip% shadow x0 y0 x1 y1 width height))))
