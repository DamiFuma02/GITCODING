  ...
  (lambda (base stringa)    ;stringa = stringa di NUMERO INTERO 
    (let ((k (- (string-length stringa) 1)))
      (if (= k 0)
                ...
                (+ 
                    (* (string-length base) (toDec base (substring stringa 0 k)))     ;funzione ricorsiva a cui viene passata la stringa senza l'ultimo carattere,  che verrà sommato progressivamente
